...
class BalanceTracker:
    ...
    def is_stale(self, max_age_minutes: int = 5) -> bool:
        """
        Check if balance data is too old (thread-safe)
    ...
    def get_age_minutes(self) -> float:
        """Get age of current balance data in minutes (thread-safe)"""
        with self._lock:
            if self.last_update is None:
                return float('inf')
    ...
    def calculate_risk_amount(self, risk_percentage: float) -> float:
        """
        Calculate risk amount in account currency (thread-safe)
    ...
    def get_balance_info(self) -> Dict[str, Any]:
        """
        Get comprehensive balance information (thread-safe)
    ...
    def get_balance_summary(self) -> str:
        """Get human-readable balance summary (thread-safe)"""
        with self._lock:
    ...
    def get_recent_history(self, limit: int = 10) -> list:
        """
        Get recent balance change history (thread-safe)
    ...
    def validate_balance(self, min_balance: float = 1000) -> Dict[str, Any]:
        """
        Validate current balance for trading (thread-safe)
    ...
    def reset(self, new_balance: float = 0):
        """
        Reset balance tracker (for testing, thread-safe)
